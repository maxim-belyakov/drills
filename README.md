...
| 2026-08-23 | timed build 1, order report | ⏱ | 🔁 4 of 8 in 45 min (12 + 33 after a break). **All four "how it ran" checks green on the first run:** 24 calls, never above 5 in flight, genuinely parallel, 232 ms. Batching by 5 was his own choice and it is correct. The four reds are not concurrency: returned an array instead of the object (byStatus and failed were both already right), summed every order instead of paid only, topCustomers never started. One unblock at minute 12 - the `list.map(fn)` -> `allSettled` shape. Eleventh sighting of green for the wrong reason: positions used as ids, which works only because ids happen to be 1..24 |
